...
int main()
{
    ...
    vett_id = shmget(IPC_PRIVATE,NUM_ELEMENTI*sizeof(int),IPC_CREAT|0644);/* TBD: usare shmget() per creare un vettore di interi su 
    ...
    *buffer = INT_MAX;
    ...
    for (int i=0;i<NUM_PROCESSI;i++){
        ...
        if(pid==0){
            figlio(vettore,buffer,sem_id,i*NUM_ELEMENTI/NUM_PROCESSI,NUM_PROCESSI);
            exit(0);
        }
    }
