...
void leave_monitor(Monitor *m){
    if(*(m->urgent_count)>0) Signal_Sem(m->urgent_sem,0);
...
void wait_condition(Monitor *m, int num_var){
    m->cond_counts[num_var]=m->cond_counts[num_var]+1;

    if(*(m->urgent_count)>0) Signal_Sem(m->urgent_sem,0);
    else Signal_Sem(m->mutex,0);

    Wait_Sem(m->id_conds,num_var);
